		...
		<div id="confirmImgDiv_{{dataid}}" style="width:100%;height: 100%;position: fixed;top:0;left: 0;display: none;z-index: 999;background-color: gray;text-align: center;">
			...
			<div class="mui-content-padded edit-div-content">
				<div class="flex-container" style="">
					...
					<a href="javascript:void(0);" @tap="confirmImg()">
	...
	export default {
		...
		methods: {
			...
			confirmImg() {
				...
				for(var file of files) {
					...
					plus.io.resolveLocalFileSystemURL(relativePath, function(entry) {
						...
						that.uploadImage(that.pictures, that.callback);
						$('#confirmImgDiv_' + that.dataid).css('display', 'none');
						$('#confirmImg_' + that.dataid).attr('src', '');
					}, function(e) {
						// 文件不存在
			...
			uploadImage: function(pictures, callback) {
				var server = api.APIS.common.uploadImage;
				log.log('上传图片:' + server);
				if(files.length <= 0) {
					...
					plus.io.resolveLocalFileSystemURL(localurl, function(entry) {
						entry.file(function(file) {
							...
							if(size > img_size) {
								// 大于指定大小，进行压缩
								var fileName = localurl.substring(localurl.lastIndexOf('/') + 1, localurl.length);
								var relativePath = '_doc/camera/zip' + fileName;
								var dstlocalurl = plus.io.convertLocalFileSystemURL(relativePath);
								plus.zip.compressImage({
									src: localurl,
									dst: dstlocalurl,
				...
				plus.io.resolveLocalFileSystemURL(path, function(entry) {
					entry.file(function(file) {
						...
						if(size > img_size) {
							...
							var relativePath = '_doc/zip' + fileName;
							var dstlocalurl = plus.io.convertLocalFileSystemURL(relativePath);
