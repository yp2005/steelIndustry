<template>
	...
	<div class="deviceList">
		<p class="conditions">
			<a href="javascript: void(0)" class="haha {{filterType == 0 ? 'filterActive':''}}" @tap="favoriteFilter(0)">店铺信息</a>
			<a href="javascript: void(0)" class="haha {{filterType == 1 ? 'filterActive':''}}" @tap="favoriteFilter(1)">用工需求</a>
			<a href="javascript: void(0)" class="haha {{filterType == 2 ? 'filterActive':''}}" @tap="favoriteFilter(2)">师傅名片</a>
			<a href="javascript: void(0)" class="haha {{filterType == 3 ? 'filterActive':''}}" @tap="favoriteFilter(3)">工程信息</a>
		</p>
	...
	export default {
		...
		methods: {
			favoriteFilter(type) {
				...
				this.noresultmsg = '目前没有需要审核的' + typestr;
