    ...
    $.fn.off = function(event, selector, callback) {
        return this.each(function() {
            ...
            if (!event) { // mui(selector).off();
                delegates[_mid] &&
        delete delegates[_mid];
            } else if (!selector) { // mui(selector).off(event);
                delegates[_mid] &&
        delete delegates[_mid][event];
            } else if (!callback) { // mui(selector).off(event,selector);
                delegates[_mid] && delegates[_mid][event] &&
            ...
            if (isCreate !== false) { // 直接创建非预加载窗口
                ...
                if (options.subpages) {
                    $.each(options.subpages, function(index, subpage) {
                        ...
                        webview.append(subWebview,subpage.url);
                    });
                }
            }
        }
...
//获取webview高度
getWebviewHeight = function(options)  {
	if(options.styles){
		var top = options.styles.top ? options.styles.top : 0;
		var bottom = options.styles.bottom ? options.styles.bottom : 0;
		top = typeof(top) == "string" ? Number(top.replace('px','')) : top;
		bottom = typeof(bottom) == "string" ? Number(bottom.replace('px','')) : bottom;
		var pageOldH = screen.availHeight-top-bottom-plus.navigator.getStatusbarHeight();
	}else{
		var pageOldH= screen.availHeight;
	}
	if(options.extras){
		options.extras.pageOldH;
	}else{
		options.extras={'pageOldH':pageOldH};
	}
};
...
//      if (plus.webview.currentWebview().id === 'seller_index') {
//          window.localStorage.setItem('openwindow_seller_index', '1')
//      }
        if ($.preView().id === 'main') {
            mui.fire(plus.webview.getWebviewById('main'), 'webview_show_event', {});
        }
        if ($.preView().id === 'seller_index') {
            mui.fire(plus.webview.getWebviewById('seller_index'), 'webview_show_event', {});
        }
        $.doAction('backs');
    };
...
//      console.log('selfView.id:' + selfView.id);
        var pageList = null;
        var item = null;
        for (var i=0;i<allView.length;i++){
            item = allView[i];
            if (item){
                if (item.__uuid__ === selfView.__uuid__) {
                    pageList = allView[i === 0 ? 0 : i - 1];
                    if (pageList.id === 'buyer_supplier_index.html') {
                        pageList = plus.webview.getWebviewById('main');
                    } else if (pageList.id === 'buyer_commonpage_buyinghall_index.html') {
                        pageList = plus.webview.getWebviewById('main');
                    } else if (pageList.id === 'seller_commonpage_buyinghall_index.html') {
                        pageList = plus.webview.getWebviewById('seller_index');
                    } else if (pageList.id === 'sellerSupplierIndex') {
                        pageList = plus.webview.getWebviewById('seller_index');
                    }
                    break;
                }
        ...
        pageList = pageList || {};
        if (window.localStorage.getItem('openwindow_seller_index') === '1' && pageList.id === 'seller_index') {
            pageList = plus.webview.getWebviewById('main');
        }
        if (pageList.id === 'HBuilder') {
            pageList = plus.webview.getWebviewById('main');
        }
//      console.log(pageList.id);
        return pageList;
    };
...
(function($, window, undefined) {
  ...
  // status: "success", "notmodified", "error", "timeout", "abort", "parsererror"
    var ajaxComplete = function(status, xhr, settings) {
        settings.complete.call(settings.context, status, xhr, settings);
    };
